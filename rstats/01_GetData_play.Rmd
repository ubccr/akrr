...
res <- dbSendQuery(con, "SELECT * FROM mod_akrr.akrr_xdmod_instanceinfo where status=1 AND ((resource LIKE \"bare_metal_real%\") OR (resource LIKE \"singularity_real%\"));")
...
ggplot(full_hpcc %>% filter(walltime>5), 
       aes(y = `Average Double-Precision General Matrix Multiplication (DGEMM) Floating-Point Performance(MFLOP per Second)`, fill=resource)) +
        geom_boxplot() + facet_wrap(~ cpu_type,scales = "free")
...
hpcc_singularity <- full_hpcc %>% filter(resource=="singularity_real")
hpcc_bare_metal <- full_hpcc %>% filter(resource=="bare_metal_real")
