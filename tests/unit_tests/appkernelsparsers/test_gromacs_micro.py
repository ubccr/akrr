def test_parser(datadir):
    from akrr.appkernelsparsers.gromacs_micro_parser import process_appker_output
    ...
    assert floats_are_close(parstat_val_f(params, "App:Version"), 2018.4)
    ...
    assert floats_are_close(parstat_val_f(stats, "Core Clock Time"), 16.978)
    assert floats_are_close(parstat_val_f(stats, "Simulation Speed"), 12.336)
    assert floats_are_close(parstat_val_f(stats, "Wall Clock Time"), 1.415)
